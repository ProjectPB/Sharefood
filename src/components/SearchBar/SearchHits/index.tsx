    ...
    <div className="searchHits">
      {indexName === 'recipes' && hits.map((hit: RecipeData) => (
        <Link to={`/recipe/${hit?.objectID}`}>
          <Hit title={hit?.title} type={hit?.type} image={hit?.imageLow} hideResults={() => hideResults()} hitType="recipe" />
        </Link>
      ))}

      {indexName === 'users' && hits.map((hit: User) => (
        <Link to={`/user/${hit?.objectID}`}>
          <Hit title={hit?.displayName} avatarSrc={hit?.profilePic} hideResults={() => hideResults()} hitType="user" />
        </Link>
      ))}
    </div>
  )
